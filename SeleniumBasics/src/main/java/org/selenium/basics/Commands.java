...
public class Commands {
	...
	public void verifyDemoWebShop() {
		...
		WebElement email = driver.findElement(
				By.xpath("/html/body/div[4]/div[1]/div[4]/div[2]/div/div[2]/div[1]/div[2]/div[2]/form/div[2]/input"));
		email.sendKeys("Kottolathil@gmail.com");
		WebElement pwd = driver.findElement(
				By.xpath("/html/body/div[4]/div[1]/div[4]/div[2]/div/div[2]/div[1]/div[2]/div[2]/form/div[3]/input"));
		pwd.sendKeys("demowebshop96#A");
	...
	public void verifyRightClick() {
		...
		actions.contextClick(rightclick).build().perform();// build method is to perform chain of operations or actions
															// //perform execute the builda action
		driver.close();
	...
	public void verifyDragandDrop() {
		...
		WebElement drop = driver
				.findElement(By.xpath("//div[@id='simpleDropContainer']//div[@class='drop-box ui-droppable']"));
		Actions actions = new Actions(driver);
	...
	// src/main/resources-showin=systemexplorer-copy the file there-close-refresh
	// the project
	public void verifyFileUpload() {
		...
		choosefile.sendKeys(
				"C:\\Users\\Vedha\\git\\SeleniumBasics\\SeleniumBasics\\src\\main\\resources\\Selenium Notes_Vedha.K.docx");
	...
	public void verifyFrames() {
		...
		int nooftags = iframes.size();
		System.out.println("Number of iframe tags are:" + nooftags);
		// driver.switchTo().frame(3); //driver.switchTo().frame("frame1");
		WebElement iframe1 = driver.findElement(By.xpath("//iframe[@id='frame1']"));
		driver.switchTo().frame(iframe1);

		WebElement iframe1text = driver.findElement(By.xpath("//h1[@id='sampleHeading']"));// no need to look iframe//no of ids inside frame can use
																							
		driver.findElement(By.xpath("//h1[@id='sampleHeading']"));
		String text = iframe1text.getText();
		System.out.println("The text of iframe is:" + text);
		driver.switchTo().defaultContent();// to switch to default webpage

	}

	public void verifyhyrFrames() {
		WebDriver driver = new ChromeDriver();
		driver.get("https://www.hyrtutorials.com/p/frames-practice.html");
		driver.manage().window().maximize();
		List<WebElement> iframes = driver.findElements(By.tagName("iframe"));
		int noofframes = iframes.size();
		System.out.println("Number of frames in webpag:" + noofframes);
		
		WebElement iframe1 = driver.findElement(By.id("frm1"));
		driver.switchTo().frame(iframe1);
        WebElement coursedrp = driver.findElement(By.id("course"));
		Select iframe1coursedrpdown = new Select(coursedrp);
		iframe1coursedrpdown.selectByIndex(1);
		WebElement getcoursetext = iframe1coursedrpdown.getFirstSelectedOption();
		System.out.println("Print the course selected by the user:" + getcoursetext);
		

		
		 WebElement iframe2 = driver.findElement(By.id("frm2"));
		 driver.switchTo().frame(iframe2);
		 WebElement basiccntrldrp =driver.findElement(By.id("selectnav1")); 
		 Select cntrldrpdown = new Select(basiccntrldrp); 
		 cntrldrpdown.selectByIndex(7);
		 WebElement getbasictext = cntrldrpdown.getFirstSelectedOption();
		 System.out.println("Print the course selected by the user:" + getbasictext);
		 driver.close();
	...
	public void verifyMultipleWindowHandle()
	{
		...
		System.out.println("The handle id of the both tabs is:"+handlesid);
		Iterator<String> values= handlesid.iterator();
		while(values.hasNext())
		{
			String childid=values.next();
	...
	public static void main(String[] args) {
		...
		// obj.verifyFrames();
		//obj.verifyhyrFrames();
		obj.verifyMultipleWindowHandle();
	}
